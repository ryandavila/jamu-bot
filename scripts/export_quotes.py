...
class QuoteExporter:
    ...
    async def export_csv(self, quotes: list[Quote], output_file: Path) -> None:
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow([
                'id', 'content', 'author', 'added_by', 'guild_id', 
                'channel_id', 'created_at', 'original_timestamp'
            ])
            ...
            for quote in quotes:
                writer.writerow([
                    ...
                    quote.original_timestamp.isoformat() if quote.original_timestamp else ''
                ])
    ...
    async def export_json(self, quotes: list[Quote], output_file: Path) -> None:
        ...
        for quote in quotes:
            data.append({
                ...
                'original_timestamp': quote.original_timestamp.isoformat() if quote.original_timestamp else None
            })
...
async def main():
    ...
    parser.add_argument('-o', '--output', default=f'quotes_export_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
                       help='Output filename (without extension)')
    parser.add_argument('-g', '--guild', type=int, help='Guild ID to filter by')
    parser.add_argument('-f', '--format', choices=['csv', 'json', 'txt', 'md'], default='csv',
